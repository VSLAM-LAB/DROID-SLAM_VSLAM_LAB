...
- **`--steps`/OneCycleLR budget not scaled down for fine-tuning — mitigated.** `train.py:85-86` builds `OneCycleLR(optimizer, args.lr, args.steps, pct_start=0.01, ...)`; `args.steps` sets the *entire* shape of that schedule (warmup ends at 1% of it, everything after is the anneal-to-~0 phase), not just how long the run happens to last. The pixi task hardcodes `--steps 250000` (`pixi.toml`, `feature.droidslam-dev.tasks.train`) — sized for pretraining on all of TartanAir. Reusing it to fine-tune on a much smaller dataset is bad either way: an early checkpoint (e.g. the 10k-interval saves, `train.py:176-178`) is caught while LR is still ~99% of peak (barely annealed), while the final checkpoint has gone through many more epochs over the small dataset than appropriate. **Fixed**: added `--epochs` (`train.py:206-207`), which — once `len(db)` is known, right after `train_loader` is built and before the optimizer/scheduler are constructed (`train.py:83-88`) — computes `args.steps = len(db) * epochs / (world_size * batch)` and logs the resolved value, so the schedule is sized to actually converge within the fine-tuning run instead of being truncated or overrun. `--steps` still works directly if `--epochs` isn't passed. Still on you: pick a target `epochs` (8-15 is a reasonable starting range) and confirm the checkpoint you evaluate is the true final one, not an intermediate mid-anneal snapshot.
